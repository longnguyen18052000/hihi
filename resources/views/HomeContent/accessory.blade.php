...
<body class="hold-transition sidebar-mini layout-fixed">
    <div class="wrapper">
        ...
        <div style="margin-left: 50px;">
            ...
            <p class="mt-5"><strong>Accessory</strong></p>
            <!-- Add 'accessory -->
            <div id="accessory" class="w3-modal resgiter">
                <div class="item mb-5">
                    <div class="row">
                        ...
                        <div class="col-6 content mt-5 mb-5">
                            ...
                            <form class="w-100" action="{{ route('accessory.store')}}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div>
                                    <label><strong>name accessory</strong></label>
                                    <input type="file" required name="name_accessory_file">
                                </div>

                                <div>
                                    <label><strong>name accessory product</strong></label>
                                    <input type="text" required name="name_accessory_product">
                                </div>
                        ...
                        <div class="col-1">
                            <button class="w-100 border border-0 mt-3 bg-white" style="color:red" onclick="document.getElementById('accessory').style.display='none'"><strong>X</strong></button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <form class="d-flex align-item-center mt-3 mb-3">
                ...
                <a class="btn btn-success" onclick="document.getElementById('accessory').style.display='block'">Add accessory</a>
            </form>
            <table class="table table-bodered table-hover table-striped mt-3 mb-5">
                <thead>
                    <th>#</th>
                    <th>name accessory</th>
                    <th>name accessory product</th>
                    <th>Edit/Delete</th>
                </thead>
                <?php $no = 0 ?>
                ...
                <tr>
                    <td>{{ $no++ }}</td>
                    <td><img src="{{ asset('images/phukien/'.$accessorys->name_accessory) }}" style="height: 70px; width: 100px;"></td>
                    <td>{{ $accessorys->name_accessory_product }}</td>
                    <td>
                        <a class="btn btn-info">Edit</a>
                        <form class="d-inline" action="{{ route('accessory.delete', $accessorys->id) }}" method="POST" onsubmit="return confirm('Delete?')">
                            @csrf
                            @method("DELETE")
